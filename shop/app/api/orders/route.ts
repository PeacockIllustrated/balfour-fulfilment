...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const { error: itemsError } = await supabase.from("bal_order_items").insert(itemsWithOrderId);

    if (itemsError) {
      console.error("Supabase items insert error:", itemsError);
      // Roll back the order so we don't leave an empty shell
      await supabase.from("bal_orders").delete().eq("id", order.id);
